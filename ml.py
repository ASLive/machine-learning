...
weights = {
    ...
    # Dense Layers
    'd1': tf.get_variable('W5', shape=(7*7*32,128),#7*7*32 ; 192
            initializer=tf.contrib.layers.xavier_initializer()),
...
def train(sess, iterator, train_op, acc_op, loss):
...
def run(load_model, save_model, visualize):
    model_file_path = './model.ckpt'
    ...
    with tf.Session() as sess:
        if load_model:
            ...
            sess = train(sess, iterator, train_op, acc_op, loss)
